...
def _make_commit_body(upgrades: list) -> str:
    """Build a commit body containing the golden-path-renovate-summary marker."""
...
def _upgrade(
    *,
    ...
    current_value="1.0.0",
    new_value="1.1.0",
    package_name=ea.ALLOWED_PACKAGE,
):
    return {
...
class TestBasicApproval(unittest.TestCase):
    def test_approves_minor_dev_any_changes(self):
        ...
        self.assertTrue(ea.evaluate(commit_body, DEFAULT_RULES, stack_changes, success=True))
    ...
    def test_rejects_when_plans_failed(self):
        upgrades = [_upgrade(package_file_dir="stacks/dev/app", update_type="patch")]
        commit_body = _make_commit_body(upgrades)
        stack_changes = {"stacks/dev/app": False}
        self.assertFalse(ea.evaluate(commit_body, DEFAULT_RULES, stack_changes, success=False))
...
class TestUnknownPackage(unittest.TestCase):
    def test_rejects_unknown_package(self):
        ...
        commit_body = _make_commit_body(upgrades)
        self.assertFalse(ea.evaluate(commit_body, DEFAULT_RULES, {}, success=True))
...
class TestPatternMatching(unittest.TestCase):
    def test_first_match_wins_prod(self):
        ...
        self.assertFalse(ea.evaluate(commit_body, DEFAULT_RULES, stack_changes, success=True))
...
class TestPolicies(unittest.TestCase):
    ...
    def test_no_changes_allows_when_no_changes(self):
        ...
        commit_body = _make_commit_body(upgrades)
        stack_changes = {"stacks/dev/app": False}
        self.assertTrue(ea.evaluate(commit_body, rules, stack_changes, success=True))
    ...
    def test_no_changes_rejects_when_has_changes(self):
        ...
        commit_body = _make_commit_body(upgrades)
        stack_changes = {"stacks/dev/app": True}
        self.assertFalse(ea.evaluate(commit_body, rules, stack_changes, success=True))
...
class TestEdgeCases(unittest.TestCase):
    def test_empty_upgrades_rejects(self):
        commit_body = "<!--golden-path-renovate-summary:[]-->"
        self.assertFalse(ea.evaluate(commit_body, DEFAULT_RULES, {}, success=True))

    def test_missing_marker_rejects(self):
        commit_body = "just a normal commit message"
        self.assertFalse(ea.evaluate(commit_body, DEFAULT_RULES, {}, success=True))

    def test_empty_commit_body_rejects(self):
        self.assertFalse(ea.evaluate("", DEFAULT_RULES, {}, success=True))
